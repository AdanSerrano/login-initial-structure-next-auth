...
export function useKeyboardNavigation<TData>({
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent) => {
      ...
      switch (event.key) {
        ...
        case "Delete":
        case "Backspace":
          if (currentIndex >= 0 && currentIndex < data.length && config.onDelete) {
            event.preventDefault();
    ...
    [enabled, config, data, getRowId, onSelectRow, onExpandRow, focusRow, containerRef]
  );
  ...
  useEffect(() => {
    ...
    const handleFocus = (event: FocusEvent) => {
      const target = event.target as HTMLElement;
      const rowIndex = target.closest("[data-row-index]")?.getAttribute("data-row-index");
      if (rowIndex !== null && rowIndex !== undefined) {
        focusedRowIndex.current = parseInt(rowIndex, 10);
      }
    };
